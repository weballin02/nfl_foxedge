...
# Fully self-contained. Optional Playwright for more robust first-page render.
...
def american_to_decimal(american: int) -> float:
    ...
    if american <= -100: return 1.0 + 100.0/abs(american)
...
# Default priors by integer line and total bucket. Buckets: 'low' < 42, 'mid' 42-49, 'high' > 49.
# Values are illustrative but realistic; can be overridden by CSV upload.
EMP_SPREAD_PUSH = {
...
def load_empirical_overrides(file: Optional[bytes]) -> Tuple[dict, dict]:
    ...
    Returns (spread_dict, total_dict) nested like the defaults above. File is optional.
    """
    if not file:
...
# Replace simple formulas with table lookups (with graceful fallback)

def spread_push_prob(home_line: Optional[float], consensus_total: Optional[float], emp_spread: dict) -> float:
    if home_line is None or not is_integerish(home_line):
        return 0.0
    ...
    return spread_push_prob.__dict__.get("_fallback", 0.0)

spread_push_prob._fallback = 0.0

def total_push_prob(total_line: Optional[float], emp_total: dict) -> float:
    if total_line is None or not is_integerish(total_line):
        return 0.0
...
def kelly_with_push(dec: float, p_win: float, p_push: float) -> float:
    ...
    For stake 1, profit b=dec-1 on win, 0 on push, -1 on loss.
    f* = (b*p_win - p_loss) / b, clipped at 0.
    """
    b = dec - 1.0
    if b <= 0:
        return 0.0
...
# Helper: consensus total per matchup to scale spread push
def consensus_total_for_matchup(full_df: pd.DataFrame, matchup: str) -> Optional[float]:
    try:
        tot_lines = full_df[(full_df["matchup"] == matchup) & (full_df["market"] == "Total")]["line"].dropna()
        if tot_lines.empty:
            return None
        # Use median to avoid stray alt totals
...
def clean_odds(s: str) -> int:
    ...
    val = int(m.group(1))
    if 0 < val < 100: val = -val  # sanitize trash like "-05"
    return val

def parse_side_and_line(side_text: str) -> Tuple[str, Optional[float]]:
    s = (side_text or "").strip().lower()
    if "home" in s: return "HOME", None
    if "away" in s: return "AWAY", None
    m = re.search(r"(over|under)\s*([0-9]+(?:\.[0-9])?)", s)
    if m: return m.group(1).upper(), float(m.group(2))
    m2 = re.search(r"(home|away)\s*([+\-]?\d+(?:\.\d)?)", s)
    if m2:
        side = "HOME" if m2.group(1) == "home" else "AWAY"
        pts = float(m2.group(2)); return side, pts
    return "", None

# -------------------- HOME/AWAY inference helper --------------------
def infer_home_away_from_text(side_text: str, away_team: str, home_team: str) -> str:
    """Infer HOME/AWAY by matching the side text against team names.
    Falls back to first token matching if full name match fails."""
...
def fetch_dk_splits(event_group: int = 88808, date_token: str = "today", _nonce: int | None = None) -> pd.DataFrame:
    """
    Fetch DK Network betting splits for NFL using the live `.tb-*` DOM.
    - Uses Playwright for first page if available, then requests for subsequent pages
    - Discovers pagination via `div.tb_pagination a[href*="tb_page="]`
    - Parses Moneyline, Spread, and Total rows with odds and % handle/% bets
    - Returns DataFrame with columns expected by this app
    - If `today` is empty, auto-fallback to `tomorrow`, then `n7days`
    - _nonce: ignored; used only to vary cache key when auto-refreshing.
    """
    ...
    import requests
    ...
    def _clean(text: str) -> str:
        return _re.sub(r"opens?\s+in\s+(?:a\s+)?new\s+tab", "", text or "", flags=_re.I).strip()
        ...
        for g in games:
            ...
            for section in g.select(".tb-market-wrap > div"):
                ...
                for row in section.select(".tb-sodd"):
                    ...
                    # spread/total line if present
                    line_val = None
                    if market_name == "Spread":
                        m = _re.search(r"([+\-]?\d+(?:\.5)?)", side_txt)
                        if m:
                            try:
                        ...
                        m = _re.search(r"(Over|Under)\s+([0-9]+(?:\.[0-9])?)", side_txt, _re.I)
                        if m:
                            try:
                    ...
                    out.append({
                        ...
                        "side": side_txt,
                        "odds": odds_val,
                        "pct_handle": float(handle_pct or 0),
                        "pct_bets": float(bets_pct or 0),
                        "update_time": now,
                        "line": line_val,
                    })
        return out

    # 1) Fetch first page and discover the rest
    ...
    if not records:
        ...
        if date_token == "tomorrow":
            return fetch_dk_splits(event_group, "n7days")

    df = pd.DataFrame.from_records(records)
    if df.empty:
        # Guarantee schema when empty so downstream code doesn't KeyError
        df = pd.DataFrame(columns=[
            "matchup","game_time","market","side","odds","pct_handle","pct_bets","update_time","line"
        ])
    return df
...
def normalize_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["key"] = df["matchup"].astype(str)+"|"+df["market"].astype(str)+"|"+df["side"].astype(str)+"|"+df["odds"].astype(str)
    df = df.drop_duplicates(subset="key").drop(columns=["key"])
    # Ensure line column exists for SPREAD/TOTAL rows even if None for others
...
@dataclass
class PriceRow:
    matchup: str; game_time: str; market: str
    selection: str; line: Optional[float]; dk_price: int
...
def adjust_with_splits(p: float, pct_handle: Optional[float], pct_bets: Optional[float], weight: float) -> float:
    if weight == 0 or pct_handle is None or pct_bets is None: return p
    diff = (pct_handle - pct_bets)/100.0  # -1..+1
    z = logit(p) + weight*diff
    return logistic(z)

def price_moneyline(group: pd.DataFrame, w: float, kcap: float) -> List[PriceRow]:
    out = []
    # derive matchup teams from normalized columns
    ...
    p_h_raw, p_a_raw = american_to_prob(int(home["odds"])), american_to_prob(int(away["odds"]))
    ...
    for sel, row, p0, p in (("HOME", home, p_h0, p_h), ("AWAY", away, p_a0, p_a)):
        ...
        ev = p*(dec-1.0)-(1-p)
        out.append(PriceRow(
            matchup=row["matchup"], game_time=row["game_time"], market="ML",
            selection=sel, line=None, dk_price=int(row["odds"]),
            fair_prob=p, fair_price=prob_to_american(p), edge=p-p0,
            ev=ev, kelly=min(kcap, kelly_fraction_decimal(p, dec)),
...
def price_total(group: pd.DataFrame, w: float, kcap: float, emp_total: dict) -> List[PriceRow]:
    out: List[PriceRow] = []
    # Build dict keyed by total line (float), entries for OVER and UNDER
    ...
    for _, r in group.iterrows():
        ...
        if side in ("OVER", "UNDER") and line is not None:
            buckets.setdefault(float(line), {})[side] = r
    for line, d in buckets.items():
        ...
        for sel, row, p0, p_sel in (
            ...
            out.append(PriceRow(
                matchup=row["matchup"], game_time=row["game_time"], market="TOTAL",
                selection=sel, line=float(line), dk_price=int(row["odds"]),
                fair_prob=p_sel, fair_price=fair_price, edge=p_sel - p0,
                ev=ev, kelly=kelly,
...
def price_spread(group: pd.DataFrame, w: float, kcap: float, full_df: pd.DataFrame, emp_spread: dict) -> List[PriceRow]:
    """Pair spread sides robustly and price them with push probability.
    Bucket key is the HOME line (home favorite negative). If DK lists only team-name
    sides (no explicit 'Home/Away'), infer using matchup team strings.
    """
    out: List[PriceRow] = []
    buckets: Dict[float, Dict[str, pd.Series]] = {}
    ...
    for _, r in group.iterrows():
        side, line = parse_side_and_line(str(r.get("side")))
        # prefer numeric line parsed earlier from scraper
        line = r.get("line", line)
        if side not in ("HOME", "AWAY"):
            side = infer_home_away_from_text(str(r.get("side")), away_name, home_name)
        # need numeric line to proceed
        ...
        try:
            ...
            continue
        # Normalize bucket to HOME line: if the row is AWAY +x, home line is -x
        home_line = pts if side == "HOME" else -pts
        home_line = float(f"{home_line:.1f}")
    ...
    for home_line, d in buckets.items():
        ...
        # raw implieds from DK prices (two-way, no-push)
        p_h_raw, p_a_raw = american_to_prob(int(home["odds"])), american_to_prob(int(away["odds"]))
        p_h0, p_a0 = remove_vig_two_way(p_h_raw, p_a_raw)
        p_h = adjust_with_splits(p_h0, home.get("pct_handle"), home.get("pct_bets"), w)
        p_a = adjust_with_splits(p_a0, away.get("pct_handle"), away.get("pct_bets"), w)

        # Push probability at this integer line (0 if half-point)
        q_push = spread_push_prob(home_line, cons_total, emp_spread)

        # For each side: win prob scaled by (1 - q), loss is the remainder
        for sel, row, p0, p, line_val in (
            ...
            # Fair price must reflect push possibility
            dec_fair = fair_decimal_from_winlose(p_win, q_push)
            fair_price = american_from_decimal(dec_fair)
            dec_market = american_to_decimal(int(row["odds"]))
            ev = p_win * (dec_market - 1.0) - p_loss
            kelly = min(kcap, kelly_with_push(dec_market, p_win, q_push))
            out.append(PriceRow(
                matchup=row["matchup"], game_time=row["game_time"], market="SPREAD",
                selection=sel, line=float(0.0 if abs(line_val) < 1e-9 else line_val), dk_price=int(row["odds"]),
                fair_prob=p, fair_price=fair_price, edge=p - p0,
                ev=ev, kelly=kelly,
                notes=f"baseline={p0:.3f}; splits_w={w:.3f}; q_push={q_push:.3f}"
            ))
    return out
...
def choose_recs(rows: List[PriceRow], ev_side_min: float, ev_total_min: float) -> pd.DataFrame:
    ...
    for r in rows:
        ...
        recs.append({
            "matchup": r.matchup, "game_time": r.game_time, "market": r.market,
            "selection": r.selection, "line": r.line, "dk_price": r.dk_price,
            "fair_prob": round(r.fair_prob,4), "fair_price": r.fair_price,
            "edge": round(r.edge,4), "ev": round(r.ev,4), "kelly": round(r.kelly,4),
            "rec": rec, "notes": r.notes
        })
    df = pd.DataFrame(recs)
    cat = pd.Categorical(df["rec"], categories=["BET","PASS"], ordered=True)
    df = df.assign(_rec=cat).sort_values(["_rec","ev"], ascending=[True, False]).drop(columns=["_rec"]).reset_index(drop=True)
    return df

# -------------------- Sidebar controls --------------------
with st.sidebar:
    st.markdown("## ⚙️ Controls")
    date_token = st.selectbox("Date", ["today","tomorrow","yesterday","this week"], index=0,
                              help="DK Network often ignores date/event filters in the public page now; parser reads what's visible.")
    if date_token in ("this week",):
        # pass 'today' and let pagination bring multiple days shown by DK page
        date_token = "today"
    event_group = st.number_input("DK Event Group (NFL)", value=88808, step=1)
    splits_weight = st.slider("Splits tilt weight (logit)", min_value=-0.5, max_value=0.5, value=0.12, step=0.01,
                              help="Positive nudges toward handle>tickets; negative is contrarian.")
    ...
    market_filter = st.multiselect("Markets", ["ML","SPREAD","TOTAL"], default=["ML","SPREAD","TOTAL"])
    st.divider()
    st.markdown("**Empirical push overrides (optional CSV)**")
    emp_file = st.file_uploader("Upload push frequencies CSV", type=["csv"], help="Columns: kind(spread/total), key(int), bucket(low/mid/high), push_prob")
    st.divider()
    auto_refresh = st.toggle("Auto refresh", value=True, help="Re-fetch page periodically to capture handle/tickets velocity.")
    refresh_sec = st.slider("Refresh interval (sec)", 15, 180, 60, 5)
    st.caption("Tip: set weight slightly negative for hard contrarian. Keep thresholds tight to avoid junk bets.")

# Load empirical push tables
emp_spread_tbl, emp_total_tbl = load_empirical_overrides(emp_file.read() if emp_file is not None else None)

# -------------------- Fetch & compute --------------------
# Auto-refresh logic
refresh_count = 0
if auto_refresh:
    # Use streamlit_autorefresh helper; expose count in sidebar for visibility
...
with st.spinner("Scraping DK Network…"):
    # Pass refresh_count to vary the cache key when auto-refresh is enabled
    raw = fetch_dk_splits(event_group, date_token, _nonce=refresh_count if auto_refresh else None)

# Handle/tickets velocity
now_ts = datetime.now(pytz.timezone("America/Los_Angeles"))
raw["ts"] = now_ts
# Build a stable key for joining snapshots
raw["_key"] = raw["matchup"].astype(str)+"|"+raw["market"].astype(str)+"|"+raw["side"].astype(str)+"|"+raw["line"].astype(str)

prev = st.session_state.get("prev_snapshot")
if prev is not None and not prev.empty:
    df_prev = prev.copy()
    df_prev = df_prev[["_key","pct_handle","pct_bets","ts"]].rename(columns={"pct_handle":"pct_handle_prev","pct_bets":"pct_bets_prev","ts":"ts_prev"})
...
st.session_state["prev_snapshot"] = raw[["_key","pct_handle","pct_bets","ts"]].copy()

df = normalize_rows(raw)
# Ensure velocity columns persist
...
# Join velocity columns into rec_df
if not rec_df.empty:
    jkey = ["matchup","market","selection","line"]
    # Build a side label in df to match selection for spreads/ML and OVER/UNDER
    def side_label(row):
        s = str(row.get("side"))
        if row.get("market") == "Moneyline":
            # infer to HOME/AWAY for join
    ...
    # normalize line sign for spreads: HOME uses home_line, AWAY uses -home_line already set in recs
    tmp_line = tmp.get("line")
...
# -------------------- Header --------------------
st.markdown('<div class="hdr"><span class="pill">NFL</span><h1 style="margin:0">Fair Pricer · DK Splits</h1></div>', unsafe_allow_html=True)

# KPIs
col_k1, col_k2, col_k3, col_k4 = st.columns([1,1,1,1])
with col_k1:
    val = len(rec_df[rec_df["rec"]=="BET"]) if not rec_df.empty else 0
    st.markdown(f'<div class="kpi"><h3>Actionable Bets</h3><div class="val">{val}</div><div class="sub">EV > 0 & edge ≥ min</div></div>', unsafe_allow_html=True)
with col_k2:
    avg_ev = rec_df["ev"].mean() if not rec_df.empty else 0.0
    st.markdown(f'<div class="kpi"><h3>Avg EV</h3><div class="val">{avg_ev:.3f}</div><div class="sub">Across visible rows</div></div>', unsafe_allow_html=True)
with col_k3:
    top = rec_df.sort_values("ev", ascending=False).head(1)
    top_ev = float(top["ev"].iloc[0]) if not top.empty else 0.0
    st.markdown(f'<div class="kpi"><h3>Top EV</h3><div class="val">{top_ev:.3f}</div><div class="sub">Highest value on board</div></div>', unsafe_allow_html=True)
with col_k4:
    ts = raw["update_time"].max() if not raw.empty else None
    ts_str = ts.strftime("%-m/%-d %I:%M %p %Z") if isinstance(ts, datetime) else "n/a"
    st.markdown(f'<div class="kpi"><h3>Data Timestamp</h3><div class="val">{ts_str}</div><div class="sub">Pacific time</div></div>', unsafe_allow_html=True)
...
with lc:
    ...
    if rec_df.empty:
        st.info("No actionable rows with current thresholds/filters.")
    else:
        # Compact pretty table
        view_cols = ["matchup","game_time","market","selection","line","dk_price","fair_price","fair_prob","edge","ev","kelly","handle_vpm","bets_vpm","rec"]
        grid = rec_df[view_cols].copy()
        # Color hints via pandas Styler
        def color_ev(val):
            if pd.isna(val): return ""
            if val >= 0.01: return "background-color: rgba(0,230,184,0.15); color:#00E6B8;"
            if val <= -0.005: return "background-color: rgba(255,26,61,0.12); color:#FF1A3D;"
            return ""
        def color_rec(val):
            if val == "BET": return "background-color: rgba(24,201,126,0.16); color:#18C97E; font-weight:700;"
            return "background-color: rgba(255,215,0,0.12); color:#FFD700;"
        styler = (grid.style
                  .applymap(color_ev, subset=["edge","ev"])
...
with rc:
    ...
    if not rec_df.empty:
        ...
        st.download_button("⬇️ Export CSV", csv, file_name="nfl_fair_pricer_recs.csv", mime="text/csv", use_container_width=True)
    st.subheader("Quick Notes")
    st.markdown("""
- **De-vig baseline** from DK prices; **splits tilt** nudges fair probs by handle vs tickets.
- **EV** = p·(dec−1) − (1−p). **Kelly** is capped by sidebar control.
- Set **weight < 0** to force contrarian bias if you want to fade public money.
""")
...
if rec_df.empty:
    st.caption("Nothing to show. Loosen your filters or lower thresholds.")
else:
    # Group by matchup and render cards
    for matchup, sub in rec_df.groupby("matchup"):
        # place basic info
        gtime = sub["game_time"].iloc[0] if "game_time" in sub else ""
        c1, c2, c3 = st.columns([3,2,2])
        with c1:
            st.markdown(f'<div class="card"><h4>{matchup}</h4><div class="badge">Kickoff: {gtime}</div></div>', unsafe_allow_html=True)
        ...
        with c2:
            ...
            if not side_rows.empty:
                ...
                handle_badge = "" if pd.isna(r.get("handle_vpm")) else f"<div class='badge'>ΔHandle {r['handle_vpm']:+.2f}%/min</div>"
                bets_badge = "" if pd.isna(r.get("bets_vpm")) else f"<div class='badge'>ΔBets {r['bets_vpm']:+.2f}%/min</div>"
        ...
        with c3:
            ...
            if not tot_rows.empty:
                ...
                handle_badge_t = "" if pd.isna(t.get("handle_vpm")) else f"<div class='badge'>ΔHandle {t['handle_vpm']:+.2f}%/min</div>"
                bets_badge_t = "" if pd.isna(t.get("bets_vpm")) else f"<div class='badge'>ΔBets {t['bets_vpm']:+.2f}%/min</div>"
                ...
                st.markdown('<div class="card"><h4>Best Total</h4><div class="badge pass">None</div></div>', unsafe_allow_html=True)

        # Full rows for this matchup
...
# -------------------- Footer --------------------
st.markdown("---")
st.caption("Source: DK Network betting splits. Event Group defaults to 88808 (NFL). Use pagination on DK page implicitly handled by the scraper.")